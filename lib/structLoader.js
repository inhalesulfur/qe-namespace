module.exports.loadQlikEngineStructs = loadQlikEngineStructs;
...
var antiStartN = /^\n/g;
var antiEndComma = /,$/g;
var regOneOf = /One of/g;
var regArrayOf = /Array of/g;
...
function loadQlikEngineStructs(version){
		...
		return new Promise((resolve, reject) => {
			fs.writeFile(pagePath, JSON.stringify(attrs, null, 4), function(err){
				if (err) reject(err);
					...
					savedPages++;
					console.log("json saved total json saved: "+savedPages+"/"+totalPages + " " +name);
					resolve();
				}
			})		
		});
	...
	function saveStructs(structs){
		...
		return new Promise((resolve, reject) => {
			fs.writeFile(jsonPath, JSON.stringify(structs, null, 4), (err) => {
				console.log("structs loaded: " + jsonPath);
				if (err) reject(err);
				else resolve();
			})
		});
	...
	function parsePages(structs){
		var queue = structs.map((d, i) => loadStructPage(d).then((html) => {
			loadedPages++;
			console.log("page loaded total pages loaded: "+loadedPages+"/"+totalPages + " " + d);
